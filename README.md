...
| Service | Endpoint |
|---|---|
| Paperless | http://localhost:8000 — `admin` / `paperless` |
| Connect (PL Dashboard) | http://localhost:3000 |
| Reactor API | http://localhost:4001/graphql |
| Machine simulator | http://localhost:8081 (Create Order form fix injected via nginx) |
| Gateway (stop-reason + costs APIs) | http://localhost:80 |

## The demo walk

1. Drop one of the sample POs (see `docs/demo-pdfs` in the ledger package
